...
//! # 問題 03: 遅延評価される配送パイプラインを作る
//!
//! 複数キャンパスの停留所を1本のイテレータへつなぎ、配送可能な停留所だけを
//! 必要になった分だけ選びます
//! 中間の `Vec` を作らず、イテレータを返す API へ変更してください
//!
//! `dispatchable_stops` の完成形:
//! - 戻り型を `impl Iterator<Item = ScheduledStop<'a>> + 'a` に変更する
//! - `P: FnMut(&Stop) -> bool + 'a` の境界を追加する
//! - `routes.iter()` から始め、`flat_map` で各ルートの停留所を入力順につなぐ
//! - `filter_map` で開いている、距離が測定済み、距離が上限以下の停留所だけを残す
//! - 基本条件を満たした停留所だけに呼び出し側の `predicate` を適用する
//! - `enumerate` と `map` で採用後の順番を1から付ける
//!
//! 制約:
//! - 中間の `Vec` や `collect` を使わない
//! - 停留所や文字列を複製しない
//! - 閉鎖中、距離未測定、距離超過の停留所では `predicate` を呼ばない
//! - イテレータを作っただけでは走査や `predicate` の呼び出しを始めない
...
impl<'a> ScheduledStop<'a> {
    #[allow(
        dead_code,
        reason = "完成前のスターターではパイプラインから呼ばれないため"
    )]
...
fn dispatchable_stops<'a, P>(
    ...
    max_distance_m: u32,
    predicate: P,
) -> std::iter::Empty<ScheduledStop<'a>> {
    let _ = (routes, max_distance_m, predicate);
    todo!("ルートを遅延評価する配送パイプラインへ変換してください")
}
...
fn main() {
    let routes = vec![CampusRoute::new(
        "本郷キャンパス便",
        vec![
            Stop::new("総合図書館", true, Some(120)),
            Stop::new("工学部2号館", true, Some(480)),
        ],
    )];

    let mut stops = dispatchable_stops(&routes, 500, |stop: &Stop| stop.name().contains("工学部"));
    println!("次の停留所: {:?}", stops.next());
}
