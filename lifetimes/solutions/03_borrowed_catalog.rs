...
struct MissionCatalog<'data> {
    missions: &'data [Mission],
}

impl<'data> MissionCatalog<'data> {
    fn new(missions: &'data [Mission]) -> Self {
        Self { missions }
    }

    fn find(&self, id: &str) -> Option<&'data Mission> {
        self.missions.iter().find(|mission| mission.id() == id)
    }

    fn highest_priority(&self) -> Option<&'data Mission> {
        self.missions.iter().reduce(|highest, candidate| {
    ...
    fn urgent(&self, minimum: u8) -> Vec<&'data Mission> {
        self.missions
            .iter()
            .filter(|mission| mission.priority() >= minimum)
            .collect()
    }

    fn into_missions(self) -> &'data [Mission] {
        self.missions
    }
}
