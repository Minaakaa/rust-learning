...
/// 選ばれた配送先を借用して返す
fn choose_destination<'a>(first: &'a str, second: &'a str, prefer_first: bool) -> &'a str {
    if prefer_first { first } else { second }
}

/// 経路内で最初の区切り文字より前にある地区名を借用して返す
fn district_before<'route>(route: &'route str, separator: &str) -> Option<&'route str> {
