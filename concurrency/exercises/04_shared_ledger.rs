...
#[allow(
    unused_imports,
    reason = "完成前のスターターでは poison 回復と thread 起動から参照されないため"
)]
use std::sync::PoisonError;
use std::sync::{Arc, Mutex, MutexGuard};
#[allow(
    unused_imports,
    reason = "完成前のスターターでは record_batches から参照されないため"
)]
use std::thread;
...
impl SharedLedger {
    ...
    fn lock_state(&self) -> MutexGuard<'_, LedgerState> {
        todo!("poisoned mutex でも内部の LedgerState を回収してください")
    }

    fn record(&self, delivery: Delivery) {
        todo!(
            "{} の配送 {}m を全体と robot 別に飽和加算してください",
            delivery.robot_id(),
            delivery.distance_m
        )
    }
    ...
    fn snapshot(&self) -> LedgerSnapshot {
        todo!("ロック中の状態から独立した LedgerSnapshot を作ってください")
    }

    fn record_batches(&self, batches: Vec<Vec<Delivery>>) {
        let batch_count = batches.len();
        drop(batches);
        todo!("{batch_count} 個のバッチを worker thread で記録してください")
    }
