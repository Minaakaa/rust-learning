...
#[allow(
    unused_imports,
    reason = "完成前のスターターでは channel 作成と try_recv から参照されないため"
)]
use std::sync::mpsc::{self, Receiver, TryRecvError};
#[allow(
    unused_imports,
    reason = "完成前のスターターでは producer thread 起動から参照されないため"
)]
use std::thread;
...
#[allow(
    dead_code,
    reason = "完成前のスターターでは collect_disconnected から構築されないため"
)]
enum GatherError {
...
fn collect_disconnected(receiver: Receiver<Envelope>) -> Result<Vec<Envelope>, GatherError> {
    ...
    drop(receiver);
    todo!("try_recv で切断まで読み取り、受信済みの Envelope を正規化してください")
}

fn gather_telemetry(batches: Vec<Vec<Telemetry>>) -> Vec<Envelope> {
    let batch_count = batches.len();
    drop(batches);
    todo!("{batch_count} 個のバッチを producer thread から集約してください")
}
