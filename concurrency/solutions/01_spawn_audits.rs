...
fn launch_audit(job: InspectionJob) -> JoinHandle<Result<AuditReport, AuditError>> {
    thread::spawn(move || {
        let Some(total_millivolts) = job.readings.iter().try_fold(0_u32, |total, reading| {
            total.checked_add(reading.millivolts)
        }) else {
            return Err(AuditError::SignalOverflow(job));
        };

        let InspectionJob { robot_id, readings } = job;
        let labels = readings.into_iter().map(|reading| reading.label).collect();
