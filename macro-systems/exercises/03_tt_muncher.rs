//! # 問題 03: TT muncherで小さなDSLを読む
//!
//! `mission_plan!`は、配送計画を専用のtoken列から`Vec<Mission>`へ変換します
//! 再帰macro ruleで先頭の1件を処理し、残りのtoken treeを次の呼び出しへ渡してください
//!
//! DSLの形:
//!
//! ```text
...
//! ```
//!
//! `ident`、`expr`、`tt`の役割と、semicolonがfragmentのfollow setにあることを確認します
...
macro_rules! mission_plan {
    ...
    (@parse $out:ident; mission $id:ident => $destination:expr, priority $priority:expr; $($rest:tt)*) => {{
        if std::hint::black_box(false) {
            todo!("先頭のmissionを{}へ追加して残りを再帰的に解析してください", stringify!($id))
        }
        $out.push(Mission::new(stringify!($id), String::new(), 0));
    }};
    ($($tokens:tt)*) => {{
        #[allow(
            unused_mut,
