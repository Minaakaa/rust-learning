...
//! # 問題 04: 標準の変換トレイトで入力を受け取る
//!
//! ロボットの表示名は `Label`、バッテリー残量は `BatteryPercent` という専用型で表す
//! 変換が必ず成功するか、入力によって失敗し得るかを考え、標準の変換トレイトを
//! 使い分けてください
//!
//! `Label` は任意の UTF-8 文字列を所有できるため、`&str` と `String` からの変換に
//! `From` を使います
//! 一方、`BatteryPercent` が受理できるのは `0..=100` だけなので、
//! `TryFrom` で範囲外を `BatteryError` にしてください
//!
//! `impl From<T> for U` を書くと `T: Into<U>` が、`impl TryFrom<T> for U` を書くと
//! `T: TryInto<U>` が標準ライブラリの包括実装（blanket implementation）によって
//! 利用できます
//! `Into` と `TryInto` は直接実装しません
//!
//! 仕様:
//! - `Label::from(&str)` は新しい所有文字列を作る
//! - `Label::from(String)` は受け取った所有文字列をそのまま保持する
//! - `RobotStatus::new` は `impl Into<Label>` で `&str`、`String`、`Label` を受け取る
//! - `BatteryPercent::try_from` は 0 と 100 を含む範囲だけを受理する
//! - 範囲外の値は元の `u16` を `BatteryError::OutOfRange` に残す
...
impl From<&str> for Label {
    fn from(value: &str) -> Self {
        todo!("文字列スライス {value:?} を所有する Label に変換してください")
    }
}

impl From<String> for Label {
    fn from(value: String) -> Self {
        todo!("所有文字列 {value:?} を再利用して Label に変換してください")
    }
}
...
impl TryFrom<u16> for BatteryPercent {
    ...
    fn try_from(value: u16) -> Result<Self, Self::Error> {
        todo!("残量 {value} が 0..=100 なら BatteryPercent に変換してください")
    }
}
...
impl RobotStatus {
    fn new(label: impl Into<Label>, battery: BatteryPercent) -> Self {
        // 未実装でも引数を使う形を保つための行で、実装時に削除してよい
        let _ = &label;
        todo!(
            "Into<Label> を使い、残量 {}% の RobotStatus を作ってください",
            battery.value()
        )
    }
...
fn main() {
    let battery = BatteryPercent::try_from(72).expect("72 は有効なバッテリー残量");
    let status = RobotStatus::new("図書館配送ロボット", battery);
