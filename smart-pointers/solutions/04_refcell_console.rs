...
impl RobotConsole {
    ...
    fn try_complete(&self, mission: Mission) -> Result<Receipt, CompleteError> {
        let mut state = match self.state.try_borrow_mut() {
            Ok(state) => state,
            Err(_) => return Err(CompleteError::Busy(mission)),
        };

        if mission.energy_wh() > state.battery_wh {
            return Err(CompleteError::InsufficientBattery {
                mission,
                available_wh: state.battery_wh,
            });
        }

        let completed = match state.completed.checked_add(1) {
            Some(completed) => completed,
            None => return Err(CompleteError::CountOverflow(mission)),
        };
        state.battery_wh -= mission.energy_wh();
        state.completed = completed;

        Ok(Receipt {
            mission_id: mission.into_id(),
            remaining_wh: state.battery_wh,
            completed,
        })
    }
}
