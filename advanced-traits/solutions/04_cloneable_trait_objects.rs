...
impl DispatchPolicy for PriorityPolicy {
    ...
    fn select(&mut self, missions: &[Mission]) -> Option<usize> {
        ...
        for (index, mission) in missions.iter().enumerate().skip(1) {
            if mission.priority > missions[selected].priority {
                selected = index;
            }
        }

        self.selected_ids.push(missions[selected].id.clone());
        Some(selected)
    }
...
impl DispatchPolicy for RoundRobinPolicy {
    ...
    fn select(&mut self, missions: &[Mission]) -> Option<usize> {
        ...
        let selected = self.cursor % missions.len();
        self.cursor = (selected + 1) % missions.len();
        self.selected_ids.push(missions[selected].id.clone());
        Some(selected)
    }
...
impl DispatchPlanner {
    ...
    fn plan_all(&mut self, missions: &[Mission]) -> Vec<PlanDecision> {
        self.policies
            .iter_mut()
            .map(|policy| {
                let mission_id = policy
                    .select(missions)
                    .and_then(|index| missions.get(index))
