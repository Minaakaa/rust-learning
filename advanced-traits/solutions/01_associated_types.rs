...
impl Sensor for BatterySensor {
    ...
    fn read(&mut self) -> Result<Self::Reading, Self::Error> {
        let raw = self.pending.pop_front().ok_or(BatteryError::NoReading)?;

        match u8::try_from(raw) {
            Ok(percent @ 0..=100) => Ok(BatteryReading { percent }),
            _ => Err(BatteryError::OutOfRange(raw)),
        }
    }
}
...
{
    sensor.read().map(|reading| reading.percent())
}
...
impl Sensor for CargoScanner {
    ...
    fn read(&mut self) -> Result<Self::Reading, Self::Error> {
        self.pending.pop_front().ok_or(CargoScanError::NoCargo)
    }
}
