...
//!
//! この解答はRust 1.86で安定化されたtrait upcastingを使い、旧式の`as_any` helperを
//! `Extension`へ追加しません
...
impl ExtensionRegistry {
        ...
        E: Extension,
    {
        let previous = self
            .extensions
            .insert(TypeId::of::<E>(), Box::new(extension))?;
        let erased: Box<dyn Any + Send + Sync> = previous;

        match erased.downcast::<E>() {
            Ok(previous) => Some(previous),
            Err(_) => unreachable!("TypeIdをkeyにするregistryの不変条件に反しています"),
        }
    }
    ...
    where
        E: Extension,
    {
        let extension = self.extensions.get(&TypeId::of::<E>())?;
        let erased: &dyn Any = extension.as_ref();
        erased.downcast_ref::<E>()
    }
    ...
    where
        E: Extension,
    {
        let extension = self.extensions.get_mut(&TypeId::of::<E>())?;
        let erased: &mut dyn Any = extension.as_mut();
        erased.downcast_mut::<E>()
    }
    ...
    where
        E: Extension,
    {
        let extension = self.extensions.remove(&TypeId::of::<E>())?;
        let erased: Box<dyn Any + Send + Sync> = extension;
