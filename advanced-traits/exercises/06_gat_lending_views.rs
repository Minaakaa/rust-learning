//! # 問題 06: GATで所有logからborrowed viewを貸し出す
//!
...
//!
//! 仕様:
//! - `type View<'a>: SampleView where Self: 'a`でborrowed viewの型族を宣言する
//! - `next_view`はcursor位置の記録を借用し、成功したときだけcursorを1つ進める
//! - `TelemetryLog`と`InspectionLog`は互いに異なるview型を選ぶ
//! - `lend_next`は具体的なview型を列挙しないgeneric consumerにする
//! - `next_view`と`lend_next`では記録や文字列を`Clone`せず、view用のallocationも作らない
//!
//! TODO:
//! - 2つの`next_view`で、sourceを借用するviewを順番に返す
//! - `lend_next`からsource固有の`S::View<'a>`を返す
//!
//! ヒント:
//! - GAT実装は`type View<'a> = TelemetryView<'a> where Self: 'a`の形で書ける
//! - `Vec::get`なら記録を移動せずに借用できる
//! - viewを返せなかった場合はcursorを変更しない
...
where
    S: ViewLender,
{
    // 未実行のclosureで必要なmethodと戻り値だけを型検査する
    let _required_method = |source: &'a mut S| -> Option<S::View<'a>> { source.next_view() };
    let _ = source;
    todo!("source固有のGATを保ったまま次のviewを返してください")
}
...
impl ViewLender for TelemetryLog {
    ...
    fn next_view<'a>(&'a mut self) -> Option<Self::View<'a>> {
        todo!(
            "cursor={}のtelemetryをallocationなしで貸し出してください",
            self.cursor
        )
    }
}
...
impl ViewLender for InspectionLog {
    ...
    fn next_view<'a>(&'a mut self) -> Option<Self::View<'a>> {
        todo!(
            "cursor={}のinspectionをallocationなしで貸し出してください",
            self.cursor
        )
    }
}
