...
//! # 問題 05: 遠隔測定サービスを graceful shutdown する
//!
//! bounded channel で受け付けた遠隔測定を複数の非同期 task で処理します
//! shutdown 通知を受けたら新しい送信を拒否し、すでに受け付けた仕事を失わず
//! 完了まで待つサービスを完成させてください
//!
//! 仕様:
//! - `submit_with_timeout` は送信枠を `Sender::reserve` で予約してから値を送る
//! - timeout と channel close では未送信の `TelemetryJob` をエラーで返す
//! - service は `tokio::select!` で shutdown、受信、task 完了を同時に待つ
//! - 受信した job ごとに `JoinSet::spawn` で処理 task を起動する
//! - 起動中の処理 task は `MAX_IN_FLIGHT` 個までとし、上限中は受信を止める
//! - 受信を止めることで bounded channel の backpressure を producer まで伝える
//! - 値を2倍にできない job は `ProcessingError` を返し、失敗件数に加える
//! - shutdown 受信時は `Receiver::close` で新しい予約を拒否する
//! - close 前に受け付けた buffered job は `recv` が `None` になるまで読み切る
//! - 起動済み task は成功、失敗にかかわらずすべて `join_next` で待つ
//! - 成功時の report は task の完了順ではなく sequence 順で返す
//! - `unsafe`、unbounded channel、実時間の経過に依存するテストは使わない
//!
//! ヒント:
//! - `JoinSet::join_next` は `select!` の branch が選ばれなくても結果を失わない
//! - 受信 branch の guard に `tasks.len() < MAX_IN_FLIGHT` を加える
//! - `MAX_IN_FLIGHT` は正の固定値なので、受信が永久に無効になることはない
//! - `i64::checked_mul` なら overflow を `Result` の失敗として表現できる
//! - shutdown branch は1回だけ有効にし、`receiver.close()` 後も受信 branch を残す
//! - loop の終了条件は「receiver を読み切った」かつ「JoinSet が空」
...
#[allow(
    unused_imports,
    reason = "run_telemetry_service 完成前のスターターでは JoinSet を使用しないため"
)]
...
async fn submit_with_timeout(
    ...
    let _ = (sender, wait);
    todo!(
        "sequence {} の送信枠を予約し、失敗時は job を返してください",
        job.sequence()
    )
}
...
#[allow(
    dead_code,
    reason = "run_telemetry_service 完成前のスターターでは処理 task を起動しないため"
)]
...
async fn run_telemetry_service(
    ...
    let _ = (&mut receiver, &mut shutdown, MAX_IN_FLIGHT);
    todo!("shutdown、受信、task 完了を select! で待ち、graceful shutdown してください")
}
